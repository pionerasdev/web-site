...
class rootstrap_framework_Interface {

	/**
	 * Generates the tabs that are used in the options menu
	 */
	static function rootstrap_tabs() {
		$counter = 0;
	 ...
	 */
	static function rootstrap_fields() {
		...
		if ( isset( $rootstrap_settings['id'] ) ) {
			$option_name = $rootstrap_settings['id'];
		}
		else {
			$option_name = 'rootstrap';
		};
		...
		foreach ( $options as $value ) {
			...
			if ( has_filter( 'rootstrap_' . $value['type'] ) ) {
				$output .= apply_filters( 'rootstrap_' . $value['type'], $option_name, $value, $val );
			}
			...
			// Uploader
			case "upload":
				$output .= rootstrap_framework_Media_Uploader::rootstrap_uploader( $value['id'], $val, null );
			...
			case 'typography':
				...
				$typography_options = array(
					'sizes' => rootstrap_recognized_font_sizes(),
					'faces' => rootstrap_recognized_font_faces(),
					'styles' => rootstrap_recognized_font_styles(),
					'color' => true
				);
				...
				$typography_fields = apply_filters( 'rootstrap_typography_fields', $typography_fields, $typography_stored, $option_name, $value );
			...
			case 'background':
				...
				$output .= rootstrap_framework_Media_Uploader::rootstrap_uploader( $value['id'], $background['image'], null, esc_attr( $option_name . '[' . $value['id'] . '][image]' ) );
				...
				$repeats = rootstrap_recognized_background_repeat();
				...
				$positions = rootstrap_recognized_background_position();
				...
				$attachments = rootstrap_recognized_background_attachment();
			...
			case "info":
				...
				if ( isset( $value['desc'] ) ) {
					$output .= apply_filters('rootstrap_sanitize_info', $value['desc'] ) . "\n";
				}
				$output .= '</div>' . "\n";
				break;
		...
		if ( rootstrap_framework_Interface::rootstrap_tabs() != '' ) {
			echo '</div>';
		}
	}
